...
def translate_time(time_):
    ...
    s=year[3]
    year=year[0]
    timeArray = time.strptime(str(year+'-'+month+'-'+day_+' '+h+':'+m+':'+s), "%Y-%m-%d %H:%M:%S")
    timestamp = int(time.mktime(timeArray))
    return timestamp
global mins
json_tmp=json.loads('{'+data.split('}{')[0][3:]+'}')
print(json_tmp)
mins=[]
mins.append(translate_time(json_tmp['timestamp']))
mins.append(int(json_tmp['counter']))
...
import pickle
key_template=[]
with open('hpack_key_httpd.pickle','rb') as f:
...
def handle_block(data_,char2id_dict,id2char_dict):
    ...
    for i in range(len(data_)):
        ...
        for x in temp_key:
            ...
            if temp_value in re_values and x not in ['counter','timestamp']:
...
def handle_normal(json_obj,char2id_dict,id2char_dict):
    ...
    for x in range(len(tmplist)):
        ...
        if temp_key=='timestamp':
            tmp_time=translate_time(temp_value)
            temp_value=str(int(tmp_time)-mins[0])
        if temp_value in re_values and temp_key not in ['counter','timestamp']:
...
def exec(data):
    ...
    for i in range(len(data_)):
        ...
        json_obj=data_[i]
        json_obj=json.loads(json_obj)
        if 'host-ip-adress' in json_obj.keys():
            pid_[json_obj['host-ip-adress']].append(data_[i])
    return pid_
...
def addsdict(data):
    ...
    for i in range(len(data_)):
        ...
        json_obj=json.loads(json_obj)
        if 'host-ip-adress' in json_obj.keys():
            if pid_[json_obj['host-ip-adress']] != -1:
                data_processed.append(handle_block(pid_[json_obj['host-ip-adress']],char2id_dict,id2char_dict))
                pid_[json_obj['host-ip-adress']]=-1
        else:
