...
def translate_time(time_):
    ...
    month=day_[1]
    day_=day_[0]
    timeArray = time.strptime(str(year+'-'+month+'-'+day_+' '+day[0]), "%Y-%m-%d %H:%M:%S")
...
import pickle
key_template=[]
with open('hpack_key_ftp.pickle','rb') as f:
...
def handle_block(data_,char2id_dict,id2char_dict):
    ...
    for i in range(len(data_)):
        ...
        for x in temp_key:
            ...
            if temp_value in re_values and x not in ['timestamp']:
...
def handle_normal(json_obj,char2id_dict,id2char_dict):
    ...
    for x in range(len(tmplist)):
        ...
        if temp_key=='timestamp':
            tmp_time=translate_time(temp_value)
            temp_value=str(int(tmp_time)-mins[0])
        if temp_value in re_values and temp_key not in ['timestamp']:
...
def exec(data):
    ...
    for i in range(len(data_)):
        ...
        if 'client' in json_obj.keys():
            pid_[json_obj['client']].append(data_[i])
    return pid_
...
def addsdict(data):
    ...
    for i in range(len(data_)):
        ...
        json_obj=json.loads(json_obj)
        if 'client' in json_obj.keys():
            if pid_[json_obj['client']] != -1:
                data_processed.append(handle_block(pid_[json_obj['client']],char2id_dict,id2char_dict))
                pid_[json_obj['client']]=-1
        else:
