...
def translate_time(time_,mins):
    ...
    return str(year)+'-0'+str(month)+'-'+str(day)+'T'+str(hour)+':'+str(minutes)+':'+str(secons)
        ...
        for item_index in range(len_it):
            if key[item_index] in common_key:
                ...
                if key[item_index]=='timestamp':
                    ...
                    astr=str(int(translate(values2[values2_begin]).split('Z')[0])+int(mins[1]))
                    if len(astr)<6:
                        for ll in range(6-len(astr)):
                            astr='0'+astr
                    data_str=data_str+"\""+'counter'+"\":\""+astr+"Z"+translate(values2[values2_begin]).split('Z')[1]+"\","
                else:
                    if key[item_index]=='query-commands' and len(values2)==3:
                        continue
