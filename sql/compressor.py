...
def get_batch(data,bs,timesteps,num):
    data_x=[]
    len_data=int(len(data)/bs)
    for i in range(bs):
        data_x.append(data[i*len_data+num:timesteps+i*len_data+num+1])
    data_x=np.array(data_x)
    return data_x[:, :-1],data_x[:, -1:]

def predict_lstm(data, timesteps, bs, alphabet_size, model_name, final_step=False):
        ...
        if not final_step:
                ...
                X,y_original=get_batch(data,bs,timesteps,0)
                for i in range(bs):
                        for j in range(min(timesteps, num_iters)):
                                enc[i].write(cumul, X[i][j])
...
def main():
        ...
        alphabet_size = len(params['id2char_dict'])+3
        ...
        if l < len(series)-timesteps:
                data=strided_app(series[l:],timesteps+1, 1)
                X = data[:, :-1]
                Y_original = data[:, -1:]

                predict_lstm([X,Y_original], timesteps, 1, alphabet_size, args.model_name, final_step = True)
        else:
                f = open(args.temp_file_prefix+'.last','wb')
                bitout = arithmeticcoding_fast.BitOutputStream(f)
                enc = arithmeticcoding_fast.ArithmeticEncoder(32, bitout) 
